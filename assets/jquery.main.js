...
;(function($, window){
	...
	var Site = {
		options: {
			anchorScrollDuration: 1000
		},
		init: function() {
			var self = this;
			this.isMobileDevice = BrowserInfo.isTouchDevice || BrowserInfo.isWinPhoneDevice;
			$(function(){
				self.initStructure();
				self.initNavigation();
				self.attachEvents();
		...
		initStructure: function() {
			...
			this.win = $(window);
			this.page = $('body');
			this.sections = this.page.find('.slide');
			this.sectionsCount = this.sections.length;

			// init sections data
			this.sections.each(function() {
				var currentSection = $(this);
				var textContent = currentSection.find('.main');
				var textOffset = parseInt(textContent.css('paddingTop'), 10) || 0;
				currentSection.data('textContent', textContent);
				currentSection.data('textOffset', textOffset);
			});
		},
		initNavigation: function() {
			// anchor scroll navigation
			var self = this;
			this.navLinks = this.page.find('.navigation li');
			this.navLinks.click(function(e) {
				e.preventDefault();
				self.scrollToSection(self.navLinks.index(this));
			});

			// back to top anchor
			this.page.find('.to-top').click(function(e){
				e.preventDefault();
				self.scrollToSection(-1);
			});
		},
		attachEvents: function() {
			// handle scroll and resize event
			var self = this;
			this.win.on({
				resize: function() {
					self.resizeHandler();
		...
		resizeHandler: function() {
			...
			if(this.isMobileDevice) {
				this.slideHeight = this.win.height();
				this.sections.css({
				...
				this.page.css({
					height: this.slideHeight * this.sectionsCount
				});
				this.scrollHandler();
			}
		},
			...
			if(!this.isMobileDevice) {
				...
				this.sections.each(function(sectionIndex) {
					var currentSection = jQuery(this),
						previousSectionHeight = sectionIndex * self.slideHeight,
						calcTop = Math.max(0, previousSectionHeight - self.scrollTop);
					...
					if(sectionIndex > 0) {
						self.sectionInnerScroll(currentSection, self.slideHeight - calcTop);
					}
				});
			}
		},
		sectionInnerScroll: function(section, scrollValue) {
			// customize inner section behavior per section
			var textContent = section.data('textContent'),
				textOffset = section.data('textOffset'),
				scrollPercent = scrollValue / this.slideHeight;

			textContent.css('paddingTop', textOffset * scrollPercent);
		},
		scrollToSection: function(sectionIndex) {
			// smooth scroll to section
			var calcTop = this.isMobileDevice ? this.sections.eq(sectionIndex + 1).offset().top : this.slideHeight * (sectionIndex + 1);
			$('html,body').stop().animate({scrollTop: calcTop}, this.options.anchorScrollDuration);
		}
	}.init();
}(jQuery, this));
 ...
 */
$(function(){
	// wrap each word in <span> element
	$('#slide1 .main > p').each(function() {
		// grab only text nodes
		var textNodes = $.grep(this.childNodes, function(node) {
			return node.nodeType === 3 && $.trim(node.textContent);
		});
		$.each(textNodes, function(index, node) {
			var strData = $.trim(node.textContent).split(/\s/);

			$.each(strData, function(index, word){
				var spanElem = $('<span class="random-hover">').html(word).insertBefore(node);
				var spaceNode = document.createTextNode(' ');
				$(spaceNode).insertBefore(spanElem);
			});
	...
	// initialize random color on hover
	var colorAnimDuration = 250;
	var colors = ['#ff2500', '#7335ff', '#33cd84', '#00efff', '#fffb00', '#0148ff', '#ff38d1'];
	$('.random-hover').each(function() {
		var item = $(this),
			origColor = item.css('color');

		item.hover(function() {
			var randomColor = colors[randomIndex(0, colors.length - 1)];
			item.stop().animate({color:randomColor}, colorAnimDuration);
		}, function() {
			item.stop().animate({color:origColor}, colorAnimDuration);
		});
	});
